...
describe("Check list element", () => {
  ...
  it("renders the check description", () => {
    act(() => {
      render(
        <CheckListElement check={dummyCheck} setRef={dummySetRef} />,
        container
      );
    });
    expect(
      container.querySelector('[data-testid="check-description"]').textContent
    ).toEqual(dummyCheck.description);
  });
  ...
  it("prevents dispatch click events if the element is disabled", () => {
    ...
    act(() => {
      render(
        <CheckListElement
          check={{ ...dummyCheck, disabled: true }}
          setRef={dummySetRef}
          clickHandler={clickHandler}
        />,
        container
